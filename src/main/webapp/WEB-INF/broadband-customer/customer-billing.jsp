							...
							<tbody>
								<c:forEach var="invoice" items="${page.results}">
									<c:forEach var="tx" items="${transactionsList}">
										<c:if test="${tx.invoice_id==invoice.id}">
											<tr class="active">
												...
												<td>${tx.card_name}</td>
												<td>&nbsp;</td>
												<td><strong><fmt:formatNumber value="${invoice.amount_paid}" type="number" pattern="#,##0.00" /></strong></td>
												<td>&nbsp;</td>
											</tr>
										</c:if>
									</c:forEach>
									<tr class="${invoice.status=='unpaid' || invoice.status=='not_pay_off' ? danger : ''}">
										...
										<td><strong><fmt:formatNumber value="${invoice.amount_payable}" type="number" pattern="#,##0.00" /></strong></td>
										<td><strong>&nbsp;</strong></td>
										<!-- download icon -->
										<td>
											<c:if test="${invoice.invoice_pdf_path!=null}">
												<a target="_blank" href="${ctx}/broadband-customer/billing/invoice/pdf/download/${invoice.id}" data-toggle="tooltip" data-placement="bottom" data-original-title="download invoice PDF"><span class="glyphicon glyphicon-cloud-download"></span></a>
											</c:if>
										</td>
									</tr>
								</c:forEach>
										...
										<ul class="pagination">
											<c:forEach var="num" begin="1" end="${page.totalPage }" step="1">
												<li class="${page.pageNo == num ? 'active' : ''}">
													<a href="${ctx}/customer/billing/${customerSession.id}/${num}">${num}</a>
												</li>
											</c:forEach>
										</ul>
									</td>
