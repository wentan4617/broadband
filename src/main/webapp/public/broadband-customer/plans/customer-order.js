(function($) {
	...
	contract = (contract == '' ? '12 months contract' : contract);

	var prepay_month_tmpl = $('#prepay_month_tmpl'),
		prepay_months = prepay_month_tmpl.attr('data-prepay_months'),
		discount_type = prepay_month_tmpl.attr('data-discount_type');

	discount_type = (discount_type == '' ? 'plan_discount' : discount_type);

	var select_modem_tmpl = $('#select_modem_tmpl'),
		hardware_id_selected = select_modem_tmpl.attr('data-hardware_id_selected');

	hardware_id_selected = (hardware_id_selected == '' ? '0' : hardware_id_selected); //console.log(hardware_id_selected);

	var broadband_options_tmpl = $('#broadband_options_tmpl'),
		broadband_value_selected = broadband_options_tmpl.attr('data-order_broadband_type');
	...
	function flushOpenTerm() {
		...
		$('input[name="contract"][value="' + contract + '"]').iCheck('check');
	}
	...
	function flushPrepayMonth() {
		...
		console.log('flushPrepayMonth: ====================');
		console.log(o);
		$('#prepay-month').html(tmpl('prepay_month_tmpl', o));
		var discount_type_container = $('#discount_type_container');
		$('input[name="discount_type"]').iCheck({
			checkboxClass: 'icheckbox_square-green',
			radioClass: 'iradio_square-green'
		});
		$('input[name="discount_type"]').on('ifChecked', function() {
			...
			console.log('flushPrepayMonthSub: ====================');
			console.log(o);
			$('#prepay-month-sub-content').html(tmpl('prepay_month_sub_tmpl', o));
			$('input[name="prepaymonths"]').iCheck({
				checkboxClass: 'icheckbox_square-green',
				radioClass: 'iradio_square-green'
			});
			$('input[name="prepaymonths"]').on('ifChecked', function() {
				months_selected = Number(this.value);
				console.log('months_selected: ' + months_selected);
				if (months_selected == 1) {
					discount_type_container.hide('fast');
					discount_desc = '';
					prepay_months = 1;
					price.discount_price = 0;
					modem_selected = null;
				} else if (months_selected == 3) {
					discount_type_container.show('fast');
					if (select_plan_type == 'VDSL' && sale_id == '20023') {
						...
						if (modem_selected != null) {
							...
							hardware_value_selected = 'withmodem'; //console.log(hardware_value_selected);
						}
					} else {
						modem_selected = null;
						if (discount_type == 'plan_discount') {
							discount_desc = '3% off the total price of 3 months plan';
							prepay_months = 3;
							price.discount_price = parseInt(price.plan_price * 3 * 0.03);
						} else if (discount_type == 'hardware_discount') {
							discount_desc = 'Hardware Discount';
							prepay_months = 3;
							price.discount_price = 0;
						}
					}
				} else if (months_selected == 6) {
					discount_type_container.show('fast');
					modem_selected = null;
					if (discount_type == 'plan_discount') {
						discount_desc = '5% off the total price of 6 months plan';
						prepay_months = 6;
						price.discount_price = parseInt(price.plan_price * 6 * 0.05);
					} else if (discount_type == 'hardware_discount') {
						discount_desc = 'Hardware Discount';
						prepay_months = 6;
						price.discount_price = 0;
					}
				} else if (months_selected == 12) {
					discount_type_container.show('fast');
					if (select_plan_type == 'VDSL' && sale_id == '10023') {
						...
						if (discount_type == 'plan_discount') {
							discount_desc = '10% off the total price of 12 months plan';
							prepay_months = 12;
							price.discount_price = parseInt(price.plan_price * 12 * 0.10);
						} else if (discount_type == 'hardware_discount') {
							discount_desc = 'Hardware Discount';
							prepay_months = 12;
							price.discount_price = 0;
			...
			$('input[name="prepaymonths"][value="' + months_selected + '"]').iCheck('check');

		});

		$('input[name="discount_type"][value="' + discount_type + '"]').iCheck('check');
	}
	...
	function flushModems() {
		...
		$('input[name="modem"]').on('ifChecked', function() {
			...
			if (value == 'withoutmodem') {
				...
				price.save_modem_price = 0;
				modem_name = '';
			} else if (value == 'withmodem') { //console.log('id: ' + id + ", modems: " + modems.length);
				$.each(modems, function(i) {
					if (this.id == id) {
						modem_selected = this;
						if (contract == '12 months contract' || contract == '24 months contract') {

							if (select_plan_type == 'ADSL' && id == 1) {
								price.modem_price = parseInt(0);
								price.save_modem_price = parseInt(this.hardware_price);
							} else if ((select_plan_type == 'VDSL' || select_plan_type == 'UFB') && contract == '24 months contract') {
								price.modem_price = parseInt(0);
								price.save_modem_price = parseInt(this.hardware_price);
							} else {
								if (prepay_months == 1 || prepay_months == 3 || prepay_months == 6) {
									if (sale_id == '20023' && select_plan_type == 'VDSL') {
										...
										if (prepay_months == 1) {
											price.modem_price = this.hardware_price;
											price.save_modem_price = 0;
									...
									if (discount_type == 'plan_discount') {
										...
										price.modem_price = 0;
										price.save_modem_price = this.hardware_price;
									}
								}
							}
	...
	function flushApplication() {
		...
		$('input[name="connection_date"]').iCheck({
			checkboxClass: 'icheckbox_square-green',
			radioClass: 'iradio_square-green'
		});
	...
	function flushBroadbandOptions() {
		var o = {
			...
			contract: contract,
			plan: plan,
			isOpenFeePromotion: isOpenFeePromotion
		};
		$('#broadband-options').html(tmpl('broadband_options_tmpl', o));
		$('input[name="order_broadband_type"],input[name="am"]').iCheck({
			checkboxClass: 'icheckbox_square-green',
			radioClass: 'iradio_square-green'
		});
